...
Anthropic は、長時間稼働するアプリケーションの研究で、初期化と実装を分けるべきだと明確に勧めています。彼らの実験データでは、独立した初期化段階を使ったプロジェクトは、複数セッションにまたがる場面での機能完了率が、混在方式より 31% 高かったそうです。重要なのは、初期化段階に使った時間は、その後 3 〜 4 回のセッションで十分に回収できるという点です。地盤がしっかりしているほど、その上の建物は速く建ちます。
