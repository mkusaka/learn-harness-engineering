...
Anthropic の長時間稼働 agent の事例でも、feature list から 1 つの機能を選び、各 session で incremental progress を残す構成が紹介されています。これは WIP=1 に近い制約で、agent が一度に広く作りすぎて半端な状態を残す失敗を防ぐためのものです。書いた code 量ではなく、検証済みの小さな完了単位を増やすことが重要です。
