...
Anthropic の長時間稼働アプリケーション開発に関する研究でも、初期化を実装から分離することが明確に推奨されている。実験データによれば、専用の初期化フェーズを使ったプロジェクトは、混在アプローチに比べてマルチセッションのシナリオで機能完了率が 31% 高かった。重要な示唆は、初期化フェーズに投じた時間は次の 3〜4 セッションで完全に回収されるということだ。基礎がしっかりしているほど、壁は速く立ち上がる。
...
- 初期化に投じた時間は次の 3〜4 セッションで完全に回収される。これは追加コストではなく、先行投資だ。基礎が堅いほど、建物は速く立ち上がる。
