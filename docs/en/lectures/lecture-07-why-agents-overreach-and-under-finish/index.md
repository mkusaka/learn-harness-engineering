...
Anthropic の実験データもこれを直接裏づけています。「small next step」戦略、つまり WIP=1 に相当するやり方を使うエージェントは、広いプロンプトを使うエージェントよりもタスク完了率が 37% 高くなります。さらに興味深いのは、エージェントが生成したコード行数と、実際の機能完了には弱い負の相関があることです。書いたコードが増えるほど、完了する機能は減る。まさに「手に余ることに手を出す」をデータが証明しています。
