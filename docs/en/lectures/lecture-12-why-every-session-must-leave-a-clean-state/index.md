...
実データもそれを示しています。クリーンアップ戦略なしで 12 週間エージェント開発を行ったプロジェクトでは、次のように推移しました。

- 1週目: ビルド成功率 100%、テスト成功率 100%、新しいセッションの起動 5分
- 4週目: ビルド 95%、テスト 92%、起動 15分
- 8週目: ビルド 82%、テスト 78%、起動 35分
- 12週目: ビルド 68%、テスト 61%、起動 60分超

クリーンアップ戦略ありの同じプロジェクトでは、次の通りでした。

- 1週目: 100%、100%、5分
- 12週目: 97%、95%、9分

12週間後には、ビルド成功率に29ポイント、次のセッションの起動時間に85%の差が出ました。これは理論ではなく、観測された差です。
...
**進捗の次元**: 現在の進捗は機械可読な成果物に記録されているか。完了したサブタスク、その合格基準、進行中だが未完了のサブタスク、その現在状態、未着手のサブタスク。適切な進捗記録は、セッション開始時の診断時間を 60〜80% 削減します。
...
エージェントで 12 週間開発した Electron アプリを、2 つのアプローチで比較しました。

**クリーンアップ戦略なし**（対照群）: 12 週目、ビルド成功率 68%、テスト成功率 61%、新しいセッションの起動 60 分超、一時成果物 103 件。

**クリーンアップ戦略あり**（実験群）: 各セッション終了時に完全なクリーン状態チェック + 週次クリーンアップループ。12 週目、ビルド成功率 97%、テスト成功率 95%、新しいセッションの起動 9 分、一時成果物 11 件。

12 週目時点で、実験群のビルド成功率は 29 ポイント高く、テスト成功率は 34 ポイント高く、新しいセッションの起動時間は 85% 短くなりました。
