...
Anthropic の実験データも、これをそのまま裏づけています。「次の小さな一歩」戦略を使うエージェント（WIP=1 に相当）は、広いプロンプトのエージェントよりも完了率が37%高くなっています。さらに興味深いのは、エージェントが生成するコード行数は、実際の機能完了と弱い負の相関を示すことです。書いたコードが増えるほど、完了した機能は減るのです。「一口で食べきれない量を欲張るな」は、データでも確認されています。
